...
let arregloDeLaMatrizSemiResuelta = []; //esto fue agregado con el algoritmo y funcion de matriz semiresuelta
let matrizSemiResuelta = []; //esto fue agregado con el algoritmo y funcion de matriz semiresuelta
...
btnMenuPrincipal.addEventListener('click', () => {
  guardarMatrizSemiResueltaEnLocalStorage();
  guardarPartidaLS();
...
function getMatrizSemiResueltaLS() {
  return JSON.parse(localStorage.getItem(`matrizSemiResuelta-${usuarioActual}`));
}

function setMatrizSemiResueltaLS(matriz) {
  localStorage.setItem(`matrizSemiResuelta-${usuarioActual}`, JSON.stringify(matriz));
}

btnReiniciar.addEventListener("click", () => {
  if (movimientosrealizados > 0) {
    reiniciarFichas(getMatrizSemiResueltaLS());
  }
});
...
btnEmpezar.addEventListener("click", () => {
  ...
  if (movimientosrealizados === 0) {
    matriz = matrizSemiResuelta; 
    setMatrizSemiResueltaLS(matrizSemiResuelta);
  } else {
  ...
  if (btnEmpezar.textContent === "Regresar al menu principal") {
    removeMatrizSemiResueltaLS('matrizSemiResuelta');    
...
function dibujarFichas() {
  console.log(matriz);
  console.log(matrizSemiResuelta);
  matriz.forEach((fila) => {
...
function cargarLS() {
  ...
  //existenMovimientosPrevios puede obtener un valor o puede arrojar null o undefined
  //!undefined = true   !null = true
  //!!undefined = false  !!null = false
  //y si existen datos pues solo se cargan los (en este caso numero)
  ...
  if (!!existenMovimientosPrevios) {
    ...
    //SI NO EXISTEN MOVIMIENTOS CARGAR LA MATRIZSEMIRESUELTA
    if (movimientosrealizados === 0) {
      if (arrayPiezasJSON !== null) {
        //Creacion del arreglo para la matrizSemiResuelta
        const arrayPiezas = JSON.parse(arrayPiezasJSON);
        ...
        arrayPiezas.forEach((pieza, indice) => {
          ...
          if (indice !== arrayPiezas.length - 1) {
            arregloDeLaMatrizSemiResuelta.push([pieza.numeroPieza, imagen64ImageData]);
          } else {
            arregloDeLaMatrizSemiResuelta.push(["", imagen64ImageData]);
          }
        });

        pasarArregloaMatrizSemiResuelta();
      }
    } 
    else {
      cargarPartidaLS();
...
function pasarArregloaMatrizSemiResuelta() {
  let fila = [];

  for (let i = 0; i < arregloDeLaMatrizSemiResuelta.length; i++) {
    fila.push(arregloDeLaMatrizSemiResuelta[i]);

    // Si el tamaño del grupo es igual a numIngresado o si se ha alcanzado el final del arreglo original
    if (fila.length === numFilyCol || i === arregloDeLaMatrizSemiResuelta.length - 1) {
      matrizSemiResuelta.push(fila);
...
function guardarMatrizSemiResueltaEnLocalStorage() {
  localStorage.setItem(`matrizSemiResuelta-${usuarioActual}`, JSON.stringify(getMatrizSemiResueltaLS()));
}
...
function removeMatrizSemiResueltaLS(key){
