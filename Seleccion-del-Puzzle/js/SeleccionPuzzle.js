document.addEventListener('DOMContentLoaded', function () {
    ...
    imgs.forEach(img => {
        img.addEventListener('click', () => {
            ...
            //QUITAR ELEMENTOS DE CUADRICULA Y OCULTAR EL CONTENEDOR (SE PROBARON A LLEVAR ABAJO) 
            //SI ESTAS LINEAS SE DEJAN AQUI EL CANVAS NO APARECE DE MAS Y SIEMPRE SE GENERAN CUADRICULA Y SE GUARDAN EN LS
    ...
    btnDividir.addEventListener('click', (evento) => {
        ...
        if(numFilyCol !== 3 && numFilyCol !== 4 && numFilyCol !== 5){
            alert('Selecciona el tamaño del tablero')
            eliminarPiezasExistentes();
            contenedorImagenCuadricula.style.display = 'none';
            contenedorCrearPuzzle.style.display = 'none';
        }
            ...
            //PRUEBA QUE SE GUARDA Y CARGA IMG DEL LOCALSTORAGE 
                // const imagenprueba = localStorage.getItem('imagenVisible');
                // const imagenvisible = new Image();
                // imagenvisible.src = imagenprueba;
                // document.body.appendChild(imagenvisible);
    ...
    function guardarEnLS() {
        ...
        //Las siguientes lineas se movieron cuando se presione alguna imagen, desde ese momento se va a resetear la partida anterior
            // let movimientosrealizados = 0;
            // setMovimientosRealizadosLS('movimientosrealizados', movimientosrealizados);
